...
mod commands {
  ...
  #[cfg(windows)]
  fn write_raw_to_printer(printer_name: &str, doc_title: &str, data: &[u8]) -> Result<(), String> {
    ...
    if doc_id == 0 {
      ...
      return Err(win_last_error("No se pudo iniciar el documento de impresión"));
    }
    ...
    let ok_write =
      unsafe { WritePrinter(handle, data.as_ptr() as *mut _, data.len() as u32, &mut written) };
  ...
  pub fn printers_print_raw(input: RawPrintInput) -> Result<String, String> {
    ...
    if mode == "network" {
      ...
      println!("[printers_print_raw] tcp {host}:{port} bytes={}", decoded.len());
