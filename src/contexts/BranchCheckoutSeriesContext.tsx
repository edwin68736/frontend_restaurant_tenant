...
export function BranchCheckoutSeriesProvider({ children }: { children: ReactNode }) {
    ...
    async (branchId: number, force = false) => {
      ...
      const task = (async () => {
        try {
          ...
          const ordered = sortSeriesNotaVentaFirst(filterRestaurantCheckoutSeries(raw ?? []))
          cacheRef.current.set(branchId, { series: ordered, ready: true })
        } catch {
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps -- version dispara lectura de cacheRef
    [checkoutSeries, seriesMetaReady, refreshCheckoutSeries, invalidateCheckoutSeries, version],
  )
