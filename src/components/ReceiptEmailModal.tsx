...
export function ReceiptEmailModal({
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      await salesService.sendReceiptEmail(saleId, to, pdfBase64)
    ...
    <PortalModal open={open} onClose={sending ? () => {} : onClose} className="max-w-md" stacked>
      <div className="w-full rounded-2xl bg-white p-5 shadow-xl">
        ...
        <input
          ...
          autoComplete="email"
          className="mb-4 w-full rounded-xl border border-stone-200 px-3 py-2.5 text-sm text-stone-800 focus:border-rest-400 focus:outline-none focus:ring-2 focus:ring-rest-500/30"
        />
