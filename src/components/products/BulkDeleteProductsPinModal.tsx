...
import type { BulkDeleteRestaurantResult } from '@/services/products.service'

type Props = {
  open: boolean
  selectedCount: number
  onClose: () => void
  onConfirm: (reason: string, pin: string) => Promise<BulkDeleteRestaurantResult>
  onDone: (result: BulkDeleteRestaurantResult) => void
}
...
export function BulkDeleteProductsPinModal({
  ...
  const [result, setResult] = useState<BulkDeleteRestaurantResult | null>(null)
      ...
      <div className="bg-white rounded-2xl p-5 w-full space-y-3 shadow-xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-start justify-between gap-2">
          <div>
            ...
            {!result && (
              <p className="text-sm text-rest-700 font-medium mt-0.5">
                Productos seleccionados: {selectedCount}
              </p>
            )}
            {!result && (
              <p className="text-xs text-stone-500 mt-1">
                Solo se eliminarán productos sin historial (ventas, comandas, inventario, etc.).
              </p>
            )}
          </div>
        ...
        {result ? (
          <div className="space-y-4">
            {result.deleted.length > 0 && (
              <section>
                <h4 className="text-sm font-semibold text-rest-800 flex items-center gap-1.5 mb-2">
                  <CheckCircle2 size={16} />
                  Eliminados ({result.deleted.length})
                </h4>
                <ul className="space-y-1 text-sm text-stone-700">
                  {result.deleted.map((p) => (
                    <li key={p.id} className="px-2 py-1 rounded-lg bg-rest-50">
                ...
                <ul className="space-y-2 text-sm">
                  {result.blocked.map((p) => (
                    <li key={p.id} className="px-2 py-2 rounded-lg bg-red-50 border border-red-100">
                      <p className="font-medium text-stone-800">{p.name || `Producto #${p.id}`}</p>
            ...
            <button
              ...
              className="w-full py-2.5 bg-stone-800 text-white rounded-xl text-sm font-medium hover:bg-stone-900"
            >
              Cerrar
            </button>
          </div>
