...
import { Plus, Pencil, Trash2, Loader2, ImagePlus, UtensilsCrossed, RefreshCw } from 'lucide-react'
import {
  getProductImageUrl,
  productsService,
  type ComboGroup,
...
export function ProductCombosPanel({ branchId, categories = [] }: Props) {
  ...
  const load = () => {
    setLoading(true)
    productsService
      .list('', true, 1, 100, null, null, branchId ?? null, true, false, { combosOnly: true })
  ...
  /** «Generar otro»: siempre uno distinto, con el mismo formato del catálogo. */
    ...
    <div className="flex flex-col min-h-0 flex-1">
      <div className="flex items-center justify-between gap-2 mb-3 shrink-0">
        <p className="text-xs text-stone-500">
          Un combo agrupa varios productos a un precio fijo. En cocina se envía cada componente a
          su área.
        </p>
        <button
          type="button"
          onClick={openCreate}
          className="inline-flex items-center justify-center gap-1.5 px-3 py-2 bg-rest-600 text-white rounded-xl text-sm font-medium hover:bg-rest-700 shadow-sm shrink-0"
        >
          <Plus size={15} />
      ...
      {loading ? (
        <div className="flex justify-center py-10">
          <Loader2 className="w-7 h-7 animate-spin text-rest-600" />
        </div>
      ) : combos.length === 0 ? (
        <EmptyState
          icon={UtensilsCrossed}
          title="Aún no hay combos"
          description="Crea uno para vender varios productos juntos a un precio especial."
        />
      ) : (
              ...
              >
                ...
                <div className="flex items-center gap-0.5 shrink-0">
                  <button
                    ...
                    className="p-2 rounded-lg text-stone-400 hover:text-rest-600 hover:bg-stone-50"
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-h-[90vh] flex flex-col overflow-hidden">
                ...
                <div className="flex gap-3">
                  <label className="shrink-0 cursor-pointer">
                    ...
                    {imagePreview ? (
                      ...
                      <div className="w-20 h-20 rounded-xl border-2 border-dashed border-stone-300 flex flex-col items-center justify-center text-stone-400 hover:border-rest-400">
                        <ImagePlus size={18} />
                  ...
                  <div className="flex-1 min-w-0 space-y-2">
                    <input
                      ...
                      className="w-full px-3 py-2 rounded-lg border border-stone-200 text-sm focus:outline-none focus:ring-2 focus:ring-rest-500"
                    />
            ...
            <div className={`p-4 border-t border-stone-200 flex gap-2 shrink-0 ${MODAL_FOOTER_SAFE}`}>
              ...
              <button
                ...
                className="flex-1 px-4 py-2.5 rounded-xl bg-rest-600 text-white text-sm font-medium hover:bg-rest-700 disabled:opacity-50 inline-flex items-center justify-center gap-2"
              >
