  ...
  const addItem = (index: number, product: Product) => {
    ...
    const items = [
      ...g.items,
      {
        ...
        preparation_area_id: product.preparation_area_id ?? null,
        default_quantity: 1,
        max_quantity: 1,
        extra_price: 0,
              ...
              >
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] text-stone-500">
                    ...
                    {item.preparation_area ? ` · ${item.preparation_area}` : ''}
                  </p>
                </div>
...
function ComboProductPicker({ branchId, excludeProductId, excludeIds, onPick }: PickerProps) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      setLoading(true)
      productsService
        .list(term, true, 1, 20, null, null, branchId ?? null, true, false, { excludeCombos: true })
        .then((r) => {
            ...
            visible.map((p) => (
              ...
              >
                ...
                <span className="ml-2 text-xs text-stone-500">
                  ...
                  {p.preparation_area ? ` · ${p.preparation_area}` : ''}
                </span>
              </button>
            ))
          )}
