...
export function PosBarcodeScannerModal({
  ...
  return createPortal(
    <div
      className={clsx('fixed inset-0 flex flex-col', REST_CAMERA_SCANNER_Z)}
      role="dialog"
      aria-modal="true"
    ...
    >
      ...
      <div className={clsx('relative flex-1 min-h-0 flex flex-col items-center justify-center pb-safe', DRAWER_BOTTOM_WRAP_X_LG)}>
        <div className="relative w-full max-w-lg aspect-[4/3] rounded-2xl overflow-hidden bg-black shadow-2xl ring-1 ring-white/10">
          ...
          {/* Marco estilo escáner (hermano del viewport de la cámara) */}
          <div className="pointer-events-none absolute inset-0 z-10 flex items-center justify-center">
            <div className="relative w-[88%] max-w-[340px] h-[32%] max-h-[140px]">
              <span className="absolute top-0 left-0 w-8 h-8 border-t-4 border-l-4 border-rest-400 rounded-tl-lg" />
              <span className="absolute top-0 right-0 w-8 h-8 border-t-4 border-r-4 border-rest-400 rounded-tr-lg" />
              <span className="absolute bottom-0 left-0 w-8 h-8 border-b-4 border-l-4 border-rest-400 rounded-bl-lg" />
              <span className="absolute bottom-0 right-0 w-8 h-8 border-b-4 border-r-4 border-rest-400 rounded-br-lg" />
              <span className="absolute left-2 right-2 top-1/2 h-0.5 -translate-y-1/2 bg-gradient-to-r from-transparent via-rest-400 to-transparent opacity-90 animate-pulse" />
              <span
                className="absolute left-2 right-2 h-0.5 bg-rest-500/80 shadow-[0_0_12px_rgba(16,185,129,0.8)] animate-[scanLine_2s_ease-in-out_infinite]"
                style={{ top: '15%' }}
              />
            </div>
          </div>
