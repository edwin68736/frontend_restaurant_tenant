...
export function ReceiptPaperPreview({
  ...
  const logo = c?.logo_url?.trim()
    ? c.logo_url.trim().startsWith('data:')
      ? c.logo_url.trim()
      : resolvePublicAssetUrl(c.logo_url)
    : ''
  ...
  const hasIgv = printData.tax_amount > 0
  const change = receiptChangeAmount(printData)
    ...
    <div className="flex justify-center overflow-y-auto bg-stone-300/60 p-3 md:p-5 h-[min(70vh,520px)] min-h-[320px]">
      ...
      >
        ...
        <div className="text-center">
          ...
          {(showBusiness || !tradeName) && (
            <p className={clsx(!tradeName && 'font-bold uppercase')}>{businessName || 'Empresa'}</p>
          )}
          {c?.ruc && <p>RUC: {c.ruc}</p>}
          {addr && <p>{addr}</p>}
        ...
        <div className="text-center font-bold">
          <p>{getTipoComprobanteLabel(printData.sunat_code, printData.doc_type)}</p>
        ...
        <div className="space-y-0.5">
          {hasIgv && (
            <div className="flex justify-between">
              ...
              <span>{money(printData.subtotal)}</span>
            </div>
          )}
          {hasIgv && (
            <div className="flex justify-between">
              <span>IGV</span>
              <span>{money(printData.tax_amount)}</span>
            </div>
          )}
          <div className="flex justify-between font-bold">
            <span>TOTAL A PAGAR</span>
            <span>{money(printData.total)}</span>
          </div>
          {change > 0.009 && (
            <div className="flex justify-between">
              <span>Vuelto</span>
        ...
        <div className="mt-3 text-center text-stone-500">Tukichef - Sistema POS</div>
      </div>
    </div>
  )
}
