...
export function billingStatusForUI(res: BillingResult): SaleBillingStatus {
  const fromBilling = normalizeBillingStatus(res.billing_status)
  if (fromBilling === BILLING_STATUS.observed) return fromBilling
  switch (resolveManualBillingStatus(res)) {
    ...
    case 'processing':
      return fromBilling === BILLING_STATUS.pending ? BILLING_STATUS.sent : fromBilling
    default:
      return fromBilling
  }
}
...
export function manualBillingMessage(res: BillingResult): string {
  ...
  if (msg) return msg
  if (normalizeBillingStatus(res.billing_status) === BILLING_STATUS.observed) {
    return 'Comprobante aceptado con observaciones'
  }
  switch (status) {
    case 'accepted':
