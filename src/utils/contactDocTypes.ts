...
export function contactDocLabel(code: string): string {
  return CONTACT_DOC_TYPES.find((d) => d.code === toContactDocCode(code))?.label ?? code
}
...
export function contactDocConsultMinLength(code: string): number {
  const c = toContactDocCode(code)
  if (c === '6') return 11
  if (c === '1') return 8
  return 0
...
/** Normaliza el número según el tipo (solo dígitos en DNI/RUC). */
