...
/**
 * Componente de un combo ya resuelto: lo que se manda a cocina y lo que se pinta en el carrito.
 * Espejo de comboComponentPayload en el backend (internal/restaurant/service/restaurant_combos.go).
 */
...
export function defaultComboPicks(groups: ComboGroup[]): ComboPicks {
  ...
  for (const g of groups) {
    ...
    if (g.selection_type === 'fixed') {
      picks[id] = g.items.slice(0, 1).map((it) => ({
...
export function resolveComboComponents(
  ...
  for (const g of groups) {
    ...
    if (g.selection_type === 'fixed') {
      ...
      if (item) push(item.product_id, item.default_quantity || 1)
      continue
    }
 ...
 * Espejo de resolveComboOrderItem en el backend; si divergen, el POS muestra un precio
...
/** Etiqueta del componente en el carrito y la comanda: «2 x Papas fritas». */
