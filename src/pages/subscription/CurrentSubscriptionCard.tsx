...
function DetailCell({
  ...
  return (
    <div className="rounded-xl bg-stone-50 border border-stone-100 px-2.5 py-2 flex items-center gap-2.5 min-w-0">
      <span className={`shrink-0 w-7 h-7 rounded-lg flex items-center justify-center ${iconClass}`}>
        <Icon size={14} />
      </span>
      <div className="min-w-0">
        <p className="text-[10px] uppercase tracking-wide text-stone-400 font-medium leading-none">{label}</p>
        <p className={`text-sm font-semibold mt-0.5 truncate ${valueClass ?? 'text-stone-900'}`}>{value}</p>
      </div>
    </div>
  )
}
    ...
    <section className="rounded-2xl border border-stone-100 bg-white shadow-sm overflow-hidden">
      <div className="px-4 py-3.5 sm:px-5 sm:py-4">
        {/* Siempre visible, sin expandir/ocultar: el resumen del plan a la izquierda y sus
            datos (ciclo, inicio, fin, estado de pago) en fila al costado, no debajo. Mismo
            criterio que Tukifac — ambas vistas deben tener paridad. */}
            ...
            <p className="mt-1.5 text-sm text-stone-600">
              {hasDebt ? (
                <>
                  ...
                  {nextPay ? <span className="text-stone-500"> · vence {formatDate(nextPay)}</span> : null}
                </>
              ) : (
                <>
                  Cuota del período:{' '}
